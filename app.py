...
# --- FUNGSI OPTIMASI: Ekstraksi Kota Pintar ---
def extract_real_city(address):
    ...
    parts = [p for p in parts if p and not RE_POSTAL.fullmatch(p)]
    ...
    if parts:
        ...
        if RE_KECAMATAN.search(city_candidate):
            if len(parts) > 1:
...
def index():
    ...
    if db is not None:
        try:
            ...
            # Optimasi: Hapus fungsi print() agar tidak membebani server
...
def restaurant_detail(restaurant_id):
    ...
    if restaurant:
        ...
        for review in restaurant.get('reviews', []):
            star_value = int(review.get('rating', 0)) if str(review.get('rating', 0)).replace('.', '', 1).isdigit() else 0
            if 1 <= star_value <= 5:
