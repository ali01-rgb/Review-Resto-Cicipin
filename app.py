...
# --- PERFORMANCE OPTIMIZATION: Timezone Cache & Singleton ---
timezone_cache = {}
_tf = None  # Initialize on-demand (lazy loading) to prevent startup blocking
...
def get_timezone_finder():
    ...
    if _tf is None:
        try:
            ...
            app.logger.warning("Failed to initialize TimezoneFinder: %s", e)
            _tf = None
    return _tf
# -------------------------------------------------------
...
def compute_average_rating(restaurant):
    # Use pre-computed value if available (new optimization)
    if 'average_rating' in restaurant:
        return restaurant
    
    # Fallback for existing data without pre-computed field
...
def compute_open_status(restaurant):
    ...
    try:
        ...
        if latitude and longitude:
            try:
                # OPTIMIZATION: Use timezone cache to avoid repeated file I/O
                cache_key = f"{round(float(latitude), 3)}_{round(float(longitude), 3)}"
                if cache_key in timezone_cache:
                    timezone_str = timezone_cache[cache_key]
                else:
                    # Use singleton _tf instead of creating new instance
...
def search_restaurants(search_term=None, min_rating=None, max_price=None, sort_by=None, user_lat=None, user_lon=None):
    ...
    for restaurant in restaurants:
        ...
        # OPTIMIZATION: Use pre-computed review_count if available, fallback to len()
        if 'review_count' not in restaurant:
...
def index():
    ...
    if db is not None:
        try:
            # OPTIMIZATION: Consolidated queries for dashboard stats
            # Query 1: Get restaurant count
            restaurant_count = db.restaurants.count_documents({})

            # Query 2: Get total reviews count using aggregation
            ...
            # Query 3: Get unique cities (using pre-computed city field or extract it)
            all_restaurants = db.restaurants.find({}, {"address": 1, "city": 1})
            city_set = set()
            
            for res in all_restaurants:
                # OPTIMIZATION: Use pre-computed city if available
                if "city" in res and res["city"]:
                    city_set.add(res["city"].lower())
                else:
                    # Fallback: extract from address for existing data
...
def add_restaurant():
    ...
    if request.method == 'POST':
        try:
            ...
            # OPTIMIZATION: Extract and store city once when restaurant is created
            city = extract_real_city(address)
            ...
            new_restaurant = {
                ...
                "image_url": image_url,
                "city": city,  # Store city to avoid recalculation
                "reviews": [],
                "average_rating": None,  # Initialize for new restaurants
                "review_count": 0  # Initialize for new restaurants
            }
...
def edit_restaurant(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        # OPTIMIZATION: Recalculate city if address changed
        city = extract_real_city(address)
        ...
        update_fields = {
            ...
            "longitude": longitude,
            "price_range": price_range,
            "city": city  # Update city if address changed
        }
...
def add_review(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        # OPTIMIZATION: Update review_count and average_rating when new review is added
        try:
            updated_restaurant = db.restaurants.find_one({"_id": ObjectId(restaurant_id)})
            reviews = updated_restaurant.get('reviews', [])
            ...
            # Update stats fields in database
            db.restaurants.update_one(
                {"_id": ObjectId(restaurant_id)},
                {
                    "$set": {
            ...
            app.logger.warning("Failed to update restaurant stats: %s", e)
